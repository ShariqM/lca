...
if socket.gethostname() == 'redwood2':
    ...
    Pv = T.fmatrix('P')
    Mv = T.fmatrix('M')
    o  = T.dot(Pv, T.dot(Mv, av))
    t3dot = theano.function([Pv, Mv, av], o, allow_input_downcast=True)
    ...
    if runtype == RunType.Learning:
        ...
        for t in range(start_t, num_trials):
            ...
            for i in range(time_batch_size):
                ...
                if i < 80/iters_per_frame: # *** Ignore skip frames for now ***
                    ...
                    # Update Basis Functions
                    dPhi = get_veta(batch_size * t, neurons, runtype, time_batch_size) * (tdot(R, tdot(CT, ahat))

                    # Fuggg want to transform voltages not thresholded values
                    Phi = Phi + dPhi
                    Phi = tdot(Phi, np.diag(1/np.sqrt(np.sum(Phi**2, axis = 0))))

                    # Don't update Z for now
...
def Msparsify(I, Phi, CT, lambdav, u_prev=None, num_iterations=80):
    """
    LCA Inference.
    I: Image batch (dim x batch)
    Phi: Dictionary (dim x dictionary element)
    ...
    CT_Phi = tdot(CT, Phi)
    b = tdot(CT_Phi.T, I)
    G = tdot(CT_Phi.T, CT_Phi) - np.eye(M)
